...
import { materialApi, categoryApi } from '../../api/product'
...
export default function MaterialManagement() {
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const res: any = await materialApi.getList({ page, pageSize: 20 })
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingId) {
        await materialApi.update(editingId, values)
    ...
    <div>
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 24 }}>
          ...
          <Form.Item name="description" label="材质描述">
            <TextArea
              placeholder="材质特性、适用场景等说明（选填）"
              rows={3}
            />
          </Form.Item>
